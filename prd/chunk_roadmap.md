...
### 상세 작업
- [ ] `src/components/Tabs.tsx` — 탭 컴포넌트
  - 길이 / 무게 / 온도 3개 탭
  - 현재 선택 탭 시각적 강조 (포인트 컬러)
  - 탭 전환 시 콜백
- [ ] `src/components/Converter.tsx` — 변환 영역 컴포넌트
  - 왼쪽: 숫자 입력 필드 + 단위 드롭다운
  - 가운데: ⇄ 스왑 버튼
...
- [ ] `src/components/UnitSelect.tsx` — 단위 드롭다운 컴포넌트
  - 네이티브 select 사용
  - 카테고리에 맞는 단위 목록 표시
- [ ] `src/app/page.tsx` 업데이트 — 탭 + Converter 조합
- [ ] 에러 처리 (PRD 8장):
  - 같은 단위 → 입력값 그대로
